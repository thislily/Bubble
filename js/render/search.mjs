...
export async function searchPosts() {
    ...
    const searchTerm = document.getElementById('search-posts-input').value.toLowerCase();
    const filteredPosts = allPosts.filter(post => {
        ...
        return title.includes(searchTerm) || body.includes(searchTerm) || author.includes(searchTerm);
    });
